...
class machine_state_generator():
    ...
    def genMachineStates(self):
        if(self.rf_flag):
            ...
            statep1['state'] = np.where(self.rf_vaf['data.A1'] >= .75 * maxcur1, 2, statep1['state'])
            statep1['state'] = np.where((self.rf_vaf['data.A1'] <= .75 * maxcur1) & (dfp1['data.A1'] > 1.5), 1, statep1['state'])
...
if __name__=='__main__':
    data_rfo=machine_state_generator('data.yaml')
